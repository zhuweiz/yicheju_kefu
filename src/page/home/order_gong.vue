  ...
  <div class="w-100">
    ...
    <el-table :data="tableData" :max-height="maxHeight" style="width: 100%" class="orderTable">
      ...
      <el-table-column label="金额">
        <template slot-scope="scope">{{scope.row.sellPrice *scope.row.num}}</template>
      </el-table-column>

      <el-table-column label="供应商备注">
        <template slot-scope="scope">{{scope.row.supplierRemark}}</template>
      </el-table-column>
...
<script>
import { gyinslist, gyinsnew, qindanlist } from "../../request/api.js";
  ...
  methods: {
    init() {
      const data = {
        ...
        supplierId: this.xlgid,
        type: this.type
      };
      gyinslist(data).then(res => {
